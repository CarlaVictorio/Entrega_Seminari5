...
const readAllB = (req, res, next) => {
    const bookId = req.params.bookId;
    let projection = {
        author: 1
    };
    return Book_1.default.findById(bookId, projection)
        .populate('author')
        .then((books) => res.status(200).json({ books }))
        .catch((error) => res.status(500).json({ error }));
};
...
exports.default = { createBook, readBook, readAll, readAllA, updateBook, deleteBook, readAllB };
